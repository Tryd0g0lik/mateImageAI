...
### Кеш сессии
Создан файл "`middleware.py`". \
Как только "`request`" (от пользователя) попадает в backend, запускается "`middleware::RedisAuthMiddleware.__call__`".\
Тут проверяем "а есть ли кеш сессии пользователя в базе данных". Если кеша сессии:
- нет в Redis, пользователь получает профиль AnonymousUser.
- иначе получает, ранне свой закешированный объект. Данный объект выводится из кеша и присваевается.  

![user of cahce](./img/user_cache.png)


### Кеш данных пользователя
Все свйоства, которые извеняются в профиле пользователя - сперва кешируются в Redis (1)
...
3) после, с данными из модели "`Users`" работаем через нереляционную db. Данные сохраняются примерно на 27 часов. 
4) В "`01:00`" реляционная db обновляется через задачи из Celery "`project/celery.py`".
5) На этапе авторизации, "`COOKIE`" получаю ключ "`session_user`". Через ключ работаем с [кешом ссесси](#кеш-сессии) (описанный чуть выше).
